...
const Login = () => {
  ...
  const onSubmit = async ({ username, password }) => {
    ...
    const res = await handleLogin(loginData).unwrap();
    if (res?.success) {
      if (res?.result?.changePassword) {
        navigate(`/change-password?token=${res?.result?.token}`);
      }
      if (res?.result?.changePassword === false) {
        const readOnly = res?.result?.readOnly;
        const token = res?.result?.token;
        const loginName = res?.result?.loginname;
        const role = res?.result?.role;
        const site = res?.result?.site;
        dispatch(setUser({ readOnly, token, loginName, role, site }));
        const modal = [
          { banner: res?.result?.banner },
          { bannerTitle: res?.result?.bannerTitle },
        ];
      ...
      setErrorMessage(res?.error?.status?.[0]?.description);
    }
  };
